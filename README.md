...
creation is retried when `idempotent=True` and `idempotency_window` is either
non-positive (unlimited) or greater than ten seconds. Finite-window retries stop
before the next delay would reach the window. Non-idempotent stamps are sent
once. File uploads are retried only when the input is a path or a seekable
stream.
